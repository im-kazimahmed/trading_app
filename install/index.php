		...
		try {
			...
			if (strtolower($engine) == 'mariadb') {
				if ($version < 10.3) {
					...
					$response['message'] = 'MariaDB 10.3+ Or MySQL 5.7+ Required. <br> Your version is MariaDB ' . $version;
				}
			} else {
				if ($version < 5.7) {
					$response['error'] = 'error';
					$response['message'] = 'MariaDB 10.3+ Or MySQL 5.7+ Required. <br> Your version is MySQL ' . $version;
				}
			}
		} catch (Exception $e) {
			$response['error'] = 'error';
...
<head>
	...
	<title>Easy Installer by Kazim Ahmed</title>
...
<body>
	...
	<footer class="py-3 text-center bg--dark border-top border-primary">
		<div class="container">
			<p class="m-0 font-weight-bold">&copy;<?php echo Date('Y') ?> - All Right Reserved by <a href="https://kazimahmed.com/">Kazim Ahmed</a></p>
		</div>
	</footer>
